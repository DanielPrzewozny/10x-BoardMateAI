...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    if (!result.success) {
      return new Response(JSON.stringify({ error: "Nieprawidłowe dane wejściowe", details: result.error.format() }), {
        status: 400,
        headers: { "Content-Type": "application/json" },
      });
    }
    ...
    const response = await openRouterService.generateResponse(message, responseFormat as ResponseFormat);
