...
export const GET: APIRoute = async ({ request }) => {
  try {
    ...
    const { data: profile, error } = await supabaseClient.from("profiles").select("*").eq("UserId", userId).single();
...
export const PATCH: APIRoute = async ({ request }) => {
  try {
    ...
    const { data: existingProfile } = await supabaseClient.from("profiles").select("*").eq("UserId", userId).single();
