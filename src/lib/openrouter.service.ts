...
export class OpenRouterService {
  ...
  private async fetchWithRetry(url: string, options: RequestInit, retries = 0): Promise<Response> {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `HTTP error ${response.status}: ${errorData ? JSON.stringify(errorData) : response.statusText}`
        );
      }
  ...
  public async generateResponse(
    ...
    const messagesWithSystem = this.systemMessage ? [systemMessage, ...messages] : messages;
    ...
    if (this.debugMode) {
      console.log(`OpenRouter żądanie do modelu ${this.modelName}:`, {
        endpoint,
        headers: Object.keys(this.defaultHeaders),
    ...
    try {
      ...
      throw new Error(
        `Nie udało się uzyskać odpowiedzi z OpenRouter: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
