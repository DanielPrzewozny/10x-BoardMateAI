...
export class HistoryService {
  ...
  async getHistory(userId: string, filters: HistoryFilters) {
    ...
    if (error) {
      throw new Error(`Błąd podczas pobierania historii: ${error.message}`);
    }
  ...
  async createHistory(userId: string, input: CreateHistoryInput) {
    ...
    if (error) {
      throw new Error(`Błąd podczas dodawania wpisu do historii: ${error.message}`);
    }
